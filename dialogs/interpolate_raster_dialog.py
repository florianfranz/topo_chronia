...
class InterpolateRasterDialog(QtWidgets.QDialog, FORM_CLASS):
    def __init__(self):
        ...
        self.sea_level_tools = SeaLevel(self.base_tools)
        self.raster_tools = RasterTools(self.base_tools)
    ...
    def create_available_age_list(self):
        ...
        QgsMessageLog.logMessage(
            f"ListWidget Items: {self.nodes_age_listWidget.count()}",
            "Interpolate Raster",
            Qgis.Info
        )
    ...
    def interpolate_raster(self):
        """
        Interpolates a raster using the QGIS TIN method...
        """
        ...
        for age in self.age_values:
            worker = ThreadedWorker(self.raster_tools.generate_raster_all_in_one, age, progress_enabled=True)
    ...
    def process_next_age(self):
        ...
        # NOTE: ensure the actual code has '>=', not HTML '&gt;=' if it was a paste artifact
    ...
    def correct_water_load(self):
        ...
        for age in self.age_values:
            worker = ThreadedWorker(self.sea_level_tools.correct_water_load_newest, age, progress_enabled=True)
            worker.signals.finished.connect(self.thread_finished)
            worker.signals.error.connect(self.thread_error)
            self.threads.append(worker)
            QThreadPool.globalInstance().start(worker)
    ...
    def interpolate_final_raster(self):
        """
        Performs the final raster interpolation using the water load corrected elevation values.
        """
        output_folder_path = self.base_tools.get_layer_path("Output Folder")
        ...
        for age in self.age_values:
            worker = ThreadedWorker(self.raster_tools.perform_final_raster_interpolation, age, progress_enabled=True)
            worker.signals.finished.connect(self.thread_finished)
            worker.signals.error.connect(self.thread_error)
            self.threads.append(worker)
            QThreadPool.globalInstance().start(worker)

            # NOTE: ensure actual code uses '>='
            while QThreadPool.globalInstance().activeThreadCount() > 0:
                QApplication.processEvents()

            z_full_volume, area_full_volume, initial_volume, initial_area = self.sea_level_tools.adjust_sea_level(
                age, corrected=1
            )
