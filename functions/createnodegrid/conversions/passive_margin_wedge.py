    ...
    def passive_margin_wedge_to_nodes(self,age):
        ...
        for passive_margin_feature in PM_multipoints.getFeatures():
            ...
            if len(multi_point) < 2:
                ...
                for i in range(len(multi_point)):
                    ...
                    if profile_geometry:
                        cont_included_profile_geometry = self.feature_conversion_tools.cut_profile_spi(profile_geometry, self.continent_polygons_layer, "keep outside", "positive", age, False)
                        if cont_included_profile_geometry:
                            final_profile_geometry = self.feature_conversion_tools.check_profile_intersection(cont_included_profile_geometry,spatial_index_profiles, geometry_dict_profiles)
        ...
        for profile_feature in PMW_profiles.getFeatures():
            ...
            for point in multi_point:
                ...
                z_with_sed = self.pm_tools.passive_margin_profile_clean(distance,feature_age,raster_depth,ridge_depth,y_wedge,x_wedge,y_crest,x_crest,continent_y)
                ...
                h_sed = self.sed_tools.full_sediment_thickness(wedge_sed_thick)
                rho_sed = self.sed_tools.rho_sed(h_sed)
